  ...
  try {
    ...
    const romaneos = await db.romaneo.findMany({
      where: {
        ...
        ...(tipo !== 'todos' && { 
          tropa: {
            especie: tipo.toUpperCase() as Especie
          }
        })
      },
      include: {
    ...
    if (tipo !== 'todos') {
      whereClause.especie = tipo.toUpperCase() as Especie
    }
    ...
    const rendimientos = tropasConRomaneo.map(tropa => {
      ...
      const pesoMediaTotal = pesoVivoTotal * 0.52 // Aproximación del rinde típico (52%)
      const rinde = pesoVivoTotal > 0 ? (pesoMediaTotal / pesoVivoTotal) * 100 : 0
